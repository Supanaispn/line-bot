...
var _model = {
    ...
    pushMessage: async function (body, headers) {
        try {
            ...
            let res = await client.pushMessage("U8da5970f4ec3ae521eee19232943fc83", LINE_BODY);
    ...
    multiCast: async function (body) {
        try {
            ...
            let userID = ["U8da5970f4ec3ae521eee19232943fc83", "U8da5970f4ec3ae521eee19232943fc83"];
    ...
    fasttrade: async function (body) {
        try {
            ...
            const USER_ID = ["U8da5970f4ec3ae521eee19232943fc83", "Ue2a8d58c521ed174c7b5b40dcac413c1"];
    ...
    covid: async function (body) {
        try {
            ...
            let { data } = await axios.get(
                "https://static.easysunday.com/covid-19/getTodayCases.json?fbclid=IwAR3NuyiILCxDFc1_t--UZ6v9bSXQS4RQcnbjYZSQgazVqgVW3RusKkbDFUo"
            );
            let result = data;
    ...
    switch (eventText) {
        ...
        case "bye":
            switch (source.type) {
                ...
                case "group":
                    return replyText(replyToken, "Leaving group").then(() => client.leaveGroup(source.groupId));
                case "room":
                    return replyText(replyToken, "Leaving room").then(() => client.leaveRoom(source.roomId));
            }
        default:
...
async function handleImage(message, replyToken) {
    ...
    if (message.contentProvider.type === "line") {
        const downloadPath = path.join(__dirname, `./uploaded/img/${message.id}.jpg`);
        const previewPath = path.join(__dirname, `./uploaded/img/${message.id}.jpg`);
        ...
        getContent = downloadContent(message.id, downloadPath).then((downloadPath) => {
            ...
            return {
                originalContentUrl: "https://5b5809d66b63.ngrok.io" + "/img/" + path.basename(downloadPath),
                previewImageUrl: "https://5b5809d66b63.ngrok.io" + "/img/" + path.basename(previewPath),
            };
        });
...
async function handleVideo(message, replyToken) {
    let getContent;
    if (message.contentProvider.type === "line") {
        const downloadPath = path.join(__dirname, "./uploaded/vdo/", `${message.id}.mp4`);
        const previewPath = path.join(__dirname, "./uploaded/vdo/", `${message.id}-preview.jpg`);
...
function handleAudio(message, replyToken) {
    let getContent;
    if (message.contentProvider.type === "line") {
        const downloadPath = path.join(__dirname, "./uploaded/audio/", `${message.id}.m4a`);

        getContent = downloadContent(message.id, downloadPath).then((downloadPath) => {
            return {
                originalContentUrl: baseURL + "/uploaded/" + path.basename(downloadPath),
            };
        });
    } else {
        getContent = Promise.resolve(message.contentProvider);
